...
- `apps/web/actions/styles/index.ts` -- getStyles Server Action，需添加颜色、行业、复杂度筛选参数
...
**Execution:**
- [ ] `apps/web/actions/styles/index.ts` -- 修改 getStyles 添加 colors、industries、complexities 筛选参数 -- 支持服务端筛选
- [ ] `apps/web/app/styles/style-grid.tsx` -- 创建 AdvancedFilterPanel 组件（内联） -- 实现颜色选择器、行业标签、复杂度选择器
- [ ] `apps/web/app/styles/style-grid.tsx` -- 添加筛选面板展开/收起状态管理 -- 控制面板显示
- [ ] `apps/web/app/styles/style-grid.tsx` -- 添加筛选条件 URL 同步逻辑 -- 支持分享和刷新保持
- [ ] `apps/web/app/styles/style-grid.tsx` -- 创建 FilterTags 组件（内联） -- 显示当前筛选标签（可删除）
- [ ] `apps/web/app/styles/page.tsx` -- 传递高级筛选参数给 StyleGrid -- 服务端渲染支持

**Acceptance Criteria:**
- Given 用户点击"高级筛选"按钮，When 筛选面板展开，Then 显示颜色选择器、行业标签、复杂度等选项
- Given 用户设置多个筛选条件，When 用户点击"应用筛选"，Then 系统同时应用所有选定条件
- Given 用户设置多个筛选条件，When 用户点击"应用筛选"，Then URL 同步筛选参数，显示当前筛选条件标签（可删除）
- Given 用户点击"清除筛选"按钮，When 用户想重置所有筛选，Then 系统清除所有高级筛选条件
