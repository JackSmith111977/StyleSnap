...
| 0021 | `0021_add_follow_system.sql` | ✅ 已完成 | 2026-04-06 | 关注系统 |
| 0022 | `0022_init_style_design_tokens.sql` | ✅ 已完成 | 2026-04-06 | 设计变量初始化 |
| 0023 | `0023_add_collections.sql` | ✅ 已完成 | 2026-04-06 | 合集功能 |
| 0024 | `0024_unified_favorites.sql` | ✅ 已完成 | 2026-04-06 | 统一收藏功能 |
| 0025 | `0025_add_style_tags_associations.sql` | ✅ 已完成 | 2026-04-06 | 风格标签关联 |
...
所有设计变量字段已同步到 `design_tokens` JSONB 字段，独立字段保留作为向后兼容。
...
| 日期 | 操作 | 迁移编号 | 执行人 | 备注 |
|------|------|---------|-------|------|
| 2026-04-06 | Dashboard 执行 SQL | 0020-0026 | Kei | 批量执行迁移 #0020 至 #0026 |
...
1. **迁移 #0026 执行后**：所有独立设计变量字段（`color_palette`, `fonts`, `spacing`, `border_radius`, `shadows`）的数据已同步到 `design_tokens` JSONB 字段
2. **向后兼容**：独立字段仍然保留，代码可以逐步迁移到使用 `design_tokens`
3. **代码清理**：后续可以移除临时适配代码，直接使用 `design_tokens` 字段
...
- 迁移执行指南：`docs/reference/MIGRATION_0026_EXECUTION_GUIDE.md`
- Supabase Dashboard: https://app.supabase.com

---

*最后更新：2026-04-06 - 所有迁移文件均已执行完成*
